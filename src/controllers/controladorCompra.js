...
function conversor() {
    ...
    if (isNaN(totalEnPesos)) {
      ...
      document.getElementById('conversor').innerHTML = "" + totalEnDolares.toFixed(2);
    }
  }
  function subTotal() {
    ...
    if (isNaN(precio)) {
      ...
      if (isNaN(cantidad)) {
        ...
        document.getElementById('mensaje').innerHTML =  + total;
        return total;
      }
    }
  }
...
//    for(precio=0; precio.length;precio++)
//    document.write(precio[precio]+ "")
//   const sub = carrito.reduce((acc, prod,) => acc + prod.precio,0)
//   console.log(sub)
